...
<script>
  ...
  export default {
    name: 'JSelectAppMemberModal',
    components: {},
    props: ['modalWidth', 'multi', 'memberIds', 'store', 'text'],
    data() {
      return {
        queryParam: {
          username: "",
    ...
    methods: {
      ...
      initMemberNames() {
        if (this.memberIds) {
          // 这里最后加一个 , 的原因是因为无论如何都要使用 in 查询，防止后台进行了模糊匹配，导致查询结果不准确
          let values = this.memberIds.split(',') + ','
          let param = {[this.store]: values}
          getAction('/eth_hub/appMember/getMultiMember', param).then((list)=>{
            this.selectionRows = []
            let selectedRowKeys = []
            let textArray = []
            if(list && list.length>0){
              for(let member of list){
                textArray.push(member[this.text])
                selectedRowKeys.push(member['id'])
                this.selectionRows.push(member)
              }
            }
            this.selectedRowKeys = selectedRowKeys
            this.$emit('initComp', textArray.join(','))
      ...
      handleSubmit() {
        ...
        this.getSelectMemberRows();
        // that.$emit('ok', that.selectUserRows);
        that.$emit('ok', that.selectMemberRows);
        that.searchReset(0)
        that.close();
      },
      //获取选择用户信息
      ...
      getSelectMemberRows() {
        ...
        for (let row of this.selectionRows) {
          if (this.selectedRowKeys.includes(row.id)) {
            this.selectMemberRows.push(row)
          }
        }
        this.selectMemberIds = this.selectMemberRows.map(row => row.username).join(',')
      },
      // 点击树节点,筛选出对应的用户
